...
const lines = input
  ...
  .map((e) => {
    ...
    const patternArray = pattern.split(",").map((e) => parseInt(e, 10));
    const stringDuplicate = new Array(5).fill(string).join("?");
    const patternDuplicate = new Array(5).fill(patternArray).flat();
    return { string: stringDuplicate, count: patternDuplicate };
  });
